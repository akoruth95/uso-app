    ...
    <v-container v-if="user">
        <v-layout>
            <v-flex text-xs-center align-center xs12>
                <v-avatar size="175px"><img v-bind:src="user.photoLink || staticPhotoLink"></v-avatar>
                <h1 class="headline pt-1">{{user.firstName}} <span v-if="user.nickName">"{{user.nickName}}"</span> {{user.lastName}}</h1>
    ...
    import { TEXT } from '../../utils/constants';
     ...
     export default {
        ...
        methods: {
            ...
            getProfileDetails() {
                userService.getAttendeeProfile(this.attendee.selectedAttendee.user_id).then((res) => {
                    this.user = res["data"];
